...
namespace :assets do
  ...
  task :build_frontend do
    ...
    Dir.chdir(frontend_dir) do
      ...
      raise "Frontend build failed. See pnpm output." unless system("pnpm build")
    end

    dist_dir = File.join(frontend_dir, "dist")
    raise "Frontend build directory not found at #{dist_dir}" unless File.directory?(dist_dir)
